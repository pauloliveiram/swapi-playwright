...
test('Validar o endpoint GET /starships', async ({ request }) => {
  const response = await request.get('/api/starships/');
  const responseBody = await response.json();
  expect(response.ok()).toBeTruthy();
  expect(typeof responseBody.count).toBe('number');
  expect(responseBody.next).toContain('https://swapi.dev/api/starships/?page=');
  ...
  expect(typeof responseBody.results[0].hyperdrive_rating).toBe('string');
  expect(typeof responseBody.results[0].MGLT).toBe('string');
  expect(typeof responseBody.results[0].starship_class).toBe('string');
  ...
  expect(responseBody.results[0].url).toContain('https://swapi.dev/api/starships/');
});